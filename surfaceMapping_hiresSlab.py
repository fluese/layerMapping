...
# Create single resolution script (or processing path within this script)
# Make use of ANTsPy (to get rid of ANTs)
# Get rid of MATLAB:
...
#
# In set parameters of the setup section below, you need to specify the
# folder to the BIDS directory and the subject ID of the subject you want
# to process.
#
...
# on the surface, e.g. QSM data or fMRI results. The volume must be in
# the space of the high resolution MP2RAGE slab. Soon, it will be
# possible to specify a directory for mapping onto the surface.
#
...
#
# CAN BE DONE AUTOMATICALLY BASED ON THE VOLUME GIVEN
...
#	print('*****************************************************')
#	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled.nii.gz')) and reprocess != True: #or map_reprocess_data != True:
#		print('File exists already. Skipping process.')
#		map_data = os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled.nii.gz')
#	else:
#		input_img = nb.load(map_data)
#		resampled_img = conform(input_img, out_shape=(336,448,448), voxel_size=(0.5,0.5,0.5), order=4)
#		nb.save(resampled_img, os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled.nii.gz'))
#		map_data = os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled.nii.gz')

#if map_transform_file_onto_surface:
#	print('')
#	print('*****************************************************')
#	print('* Resampling transform data to an isotropic resolution of 500 µm')
#	print('*****************************************************')
#	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_transform_data_resampled.nii.gz')) and reprocess != True: #or map_reprocess_data != True:
#		print('File exists already. Skipping process.')
#		map_data = os.path.join(out_dir, 'sub-' + sub + '_transform_data_resampled.nii.gz')
#	else:
#			input_img = nb.load(transform_data)
#			resampled_img = conform(input_img, out_shape=(336,448,448), voxel_size=(0.5,0.5,0.5), order=4)
#			nb.save(resampled_img, os.path.join(out_dir, 'sub-' + sub + '_transform_data_resampled.nii.gz'))
#			transform_data = os.path.join(out_dir, 'sub-' + sub + '_transform_data_resampled.nii.gz')
...
if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_run-02_T1w_biasCorrected_registered_to_' + sub + '_run-01_T1map_resampled_biasCorrected_masked.nii.gz')) and reprocess != True:
	print('File exists already. Skipping process.')
else:
	# Register whole brain data to high resolution slab
	os.system('antsRegistrationSyN_sensemap.sh -d 3 -n ' + CPUs + ' -f ' + T1map_slab_biasCorrected_masked + ' -m ' + T1map_biasCorrected_masked + ' -o ' + out_dir + 'sub-' + sub + '_run-01_T1map_resampled_biasCorrected_masked_registered_to_sub-' + sub + '_run-02_T1map_biasCorrected_masked_ -t sr')

	# Apply transformation to T1map of the high resolution slab
	os.system('antsApplyTransforms -d 3 -e 0 -n BSpline[4] --float --verbose -i ' + T1map_slab + ' -r ' + T1map + ' -o ' + out_dir + 'sub-' + sub + '_run-02_T1map_registered_to_' + sub + '_run-01_T1map_resampled_biasCorrected_masked.nii.gz -t ' + out_dir + 'sub-' + sub + '_run-01_T1map_resampled_biasCorrected_masked_registered_to_sub-' + sub + '_run-02_T1map_biasCorrected_masked_1InverseWarp.nii.gz -t [' + out_dir + 'sub-' + sub + '_run-01_T1map_resampled_biasCorrected_masked_registered_to_sub-' + sub + '_run-02_T1map_biasCorrected_masked_0GenericAffine.mat,1]')

	# Apply transformation to bias corrected T1map of the high resolution slab
	os.system('antsApplyTransforms -d 3 -e 0 -n BSpline[4] --float --verbose -i ' + T1map_slab_biasCorrected + ' -r ' + T1map_biasCorrected + ' -o ' + out_dir + 'sub-' + sub + '_run-02_T1map_biasCorrected_registered_to_' + sub + '_run-01_T1map_resampled_biasCorrected_masked.nii.gz -t ' + out_dir + 'sub-' + sub + '_run-01_T1map_resampled_biasCorrected_masked_registered_to_sub-' + sub + '_run-02_T1map_biasCorrected_masked_1InverseWarp.nii.gz -t [' + out_dir + 'sub-' + sub + '_run-01_T1map_resampled_biasCorrected_masked_registered_to_sub-' + sub + '_run-02_T1map_biasCorrected_masked_0GenericAffine.mat,1]')

	# Apply transformation to bias corrected T1w of the high resolution slab
	os.system('antsApplyTransforms -d 3 -e 0 -n BSpline[4] --float --verbose -i ' + T1w_slab_biasCorrected + ' -r ' + T1w_biasCorrected + ' -o ' + out_dir + 'sub-' + sub + '_run-02_T1w_biasCorrected_registered_to_' + sub + '_run-01_T1map_resampled_biasCorrected_masked.nii.gz -t ' + out_dir + 'sub-' + sub + '_run-01_T1map_resampled_biasCorrected_masked_registered_to_sub-' + sub + '_run-02_T1map_biasCorrected_masked_1InverseWarp.nii.gz -t [' + out_dir + 'sub-' + sub + '_run-01_T1map_resampled_biasCorrected_masked_registered_to_sub-' + sub + '_run-02_T1map_biasCorrected_masked_0GenericAffine.mat,1]')

# Update file names
T1w_slab_biasCorrected_reg = os.path.join(out_dir, 'sub-' + sub + '_run-02_T1w_biasCorrected_registered_to_' + sub + '_run-01_T1map_resampled_biasCorrected_masked.nii.gz')
...
############################################################################
# 5.1. Register additional data to upsampled 500 µm data
# -------------------
