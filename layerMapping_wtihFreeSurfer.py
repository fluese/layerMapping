...
# 6. Change os.system to subprocess.run
#
...
if os.path.isfile(os.path.join(FreeSurfer_path, subID, 'scripts', 'recon-all-status.log')) and reprocess != True:
	...
	os.system("recon-all -all -hires -sd FreeSurfer_path -i T1w_biasCorrected -s subID'")

###########################################################################
# 8.2. Prepare FreeSurfer segmentation for nighres
# ------------------
# Here, we move the segmentation of FreeSurfer into the original space and
# extract the white matter, gray matter, and everything else labels from
...
# then creating probability maps from the levelsets with a distance of 1 
# mm [1 - 2 mm should be good according to Pilou].
# CRUISE should also be able to work with binary segmentations. Results 
# should be compared wo each other.
print('')
print('*****************************************************')
...
###########################################################################
# 10. Region Extraction (left hemisphere)
# ------------------
...
cortex = nighres.brain.extract_brain_region(
	...
	save_data=True,
	overwrite=False, # NEEDS TO BE CHANGED TO reprocess AGAIN
	file_name='sub-' + sub + merged + '_leftHemisphere',
	output_dir=out_dir)
        
#############################################################################
# 11. Crop volume to left hemisphere
# --------------------------------
...
if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + merged + '_leftHemisphere_T1map_croppedxx.nii.gz')) and reprocess != True: # NEEDS TO BE CHANGED AGAIN
	print('Files exists already. Skipping process.')
	inside_mask = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_leftHemisphere_segmentation_wm_binary_tpc-img_cropped.nii.gz'))
	inside_proba = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_leftHemisphere_segmentation_wm_l2p-proba_cropped.nii.gz'))
	region_proba = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_leftHemisphere_segmentation_gm_l2p-proba_cropped.nii.gz'))
	background_proba = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_leftHemisphere_segmentation_ee_l2p-proba_cropped.nii.gz'))
	...
	nb.save(inside_mask, os.path.join(out_dir, 'sub-' + sub + merged + '_leftHemisphere_segmentation_wm_binary_tpc-obj_cropped.nii.gz'))
	...
	nb.save(inside_proba, os.path.join(out_dir, 'sub-' + sub + merged + '_leftHemisphere_segmentation_wm_l2p-proba_cropped.nii.gz'))
	...
	nb.save(region_proba, os.path.join(out_dir, 'sub-' + sub + merged + '_leftHemisphere_segmentation_gm_l2p-proba_cropped.nii.gz'))
	...
	nb.save(background_proba, os.path.join(out_dir, 'sub-' + sub + merged + '_leftHemisphere_segmentation_ee_l2p-proba_cropped.nii.gz'))
...
############################################################################
# 11.1. Crop additional data to left hemispehre
# -------------------
if reprocess_map_data:
	reprocess = True
...
############################################################################
# 11.2. Crop additionally transformed data to left hemispehre
# -------------------
...
#############################################################################
# 12. CRUISE cortical reconstruction (left hemisphere)
# --------------------------------
...
if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + merged + '_rightHemisphere_T1map_cropped.nii.gz')) and reprocess != True:
	...
	inside_mask = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_rightHemisphere_wm_subcort_binary_cropped.nii.gz'))
	inside_proba = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_rightHemisphere_wm_subcort_binary_cropped.nii.gz'))
	region_proba = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_rightHemisphere_gm_binary_cropped.nii.gz'))
	background_proba = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_rightHemisphere_everything_else_binary_cropped.nii.gz'))
	...
	inside_mask = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_wm_subcort_binary.nii.gz'))
	inside_proba = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_wm_subcort_binary.nii.gz'))
	region_proba = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_gm_binary.nii.gz'))
	background_proba = nb.load(os.path.join(out_dir, 'sub-' + sub + merged + '_everything_else_binary.nii.gz'))

	tmp1 = nb.load(inside_mask)
	tmp2 = tmp1.get_fdata()
	inside_mask = nb.Nifti1Image(tmp2[coord], affine=tmp1.affine, header=tmp1.header)
	nb.save(inside_mask, os.path.join(out_dir, 'sub-' + sub + merged + '_rightHemisphere_wm_subcort_binary_cropped.nii.gz'))

	tmp1 = nb.load(inside_proba)
	tmp2 = tmp1.get_fdata()
	inside_proba = nb.Nifti1Image(tmp2[coord], affine=tmp1.affine, header=tmp1.header)
	nb.save(inside_proba, os.path.join(out_dir, 'sub-' + sub + merged + '_rightHemisphere_wm_subcort_binary_cropped.nii.gz'))

	tmp1 = nb.load(region_proba)
	tmp2 = tmp1.get_fdata()
	region_proba = nb.Nifti1Image(tmp2[coord], affine=tmp1.affine, header=tmp1.header)
	nb.save(region_proba, os.path.join(out_dir, 'sub-' + sub + merged + '_rightHemisphere_gm_binary_cropped.nii.gz'))

	tmp1 = nb.load(background_proba)
	tmp2 = tmp1.get_fdata()
	background_proba = nb.Nifti1Image(tmp2[coord], affine=tmp1.affine, header=tmp1.header)
	nb.save(background_proba, os.path.join(out_dir, 'sub-' + sub + merged + '_rightHemisphere_everything_else_binary_cropped.nii.gz'))
