...
# -------------------
# Make use of ANTsPy
# Get rid of MATLAB dependencies:
...
# 1. MATLAB scripts (weightedAverage and biasCorrection)
...
# Map specific file or all data in a folder onto the surface. Requries full
# path to a NIfTI file or folder containing NIfTI files. The data could be
# another contrast (e.g. QSM) or results of processed fMRI data.
#
# The data must be in the same space as "sub-*-run-01_T1map.nii.gz".
map_data = '/tmp/luesebrink/sub-wtl/sub-wtl_run-01_T1map.nii.gz'
...
# Check if file or directory for mapping of additional data exists.
if os.path.isfile(map_data):
	map_file_onto_surface = True
	map_dir_onto_surface = False
	print('File found for mapping of additional data onto surface found!')
elif os.path.isdir(map_data):
	map_dir_onto_surface = True
	map_file_onto_surface = False
else:
	map_file_onto_surface = False
	map_dir_onto_surface = False
	print('Could not find files for surface mapping. Omitting flag!')
...
if map_file_onto_surface:
	print('')
	print('*****************************************************')
	print('* Resampling additional data to an isotropic resolution of 500 µm')
	print('*****************************************************')
	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_data_resampled.nii.gz') and reprocess != True:
		print('File exists already. Skipping process.')
		map_data = os.path.join(out_dir, 'sub-' + sub + '_data_resampled.nii.gz')
	else:
		input_img = nb.load(map_data)
		resampled_img = conform(input_img, out_shape=(336,448,448), voxel_size=(0.5,0.5,0.5), order=4)
		nb.save(resampled_img, os.path.join(out_dir, 'sub-' + sub + '_data_resampled.nii.gz'))
		map_data = os.path.join(out_dir, 'sub-' + sub + '_data_resampled.nii.gz')
...
############################################################################
# 5. Register native 700 µm to upsampled 500 µm data
# -------------------
...
print('')
print('*****************************************************')
print('* Register native 700 µm to upsampled 500 µm data')
print('*****************************************************')
...
if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_run-02_T1w_biasCorrected_registered_to_' + sub + '_run-01_T1map_biasCorrected_resampled_masked.nii.gz')) and reprocess != True:
	...
	os.system('antsRegistrationSyN_sensemap.sh -d 3 -n ' + CPUs + ' -f ' + T1map_slab_biasCorrected_masked + ' -m ' + T1map_biasCorrected_masked + ' -o ' + out_dir + 'sub-' + sub + '_run-01_biasCorrected_resampled_masked_registered_to_sub-' + sub + '_run-02_biasCorrected_masked_ -t sr')
...
if map_file_onto_surface:
	print('')
	print('*****************************************************')
	print('* Register additional data to upsampled 500 µm data')
	print('*****************************************************')
	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_data_resampled_registered_to_' + sub + '_run-01_T1map_biasCorrected_resampled_masked.nii.gz') and reprocess != True:
		print('File exists already. Skipping process.')
		map_data = os.path.join(out_dir, 'sub-' + sub + '_data_resampled_registered_to_' + sub + '_run-01_T1map_biasCorrected_resampled_masked.nii.gz')
	else:
		# Apply transformation to data to be mapped on the surface
		os.system('antsApplyTransforms -d 3 -e 0 -n BSpline[4] --float --verbose -i ' + map_data + ' -r ' + T1w_biasCorrected + ' -o ' + out_dir + 'sub-' + sub + '_data_resampled_registered_to_' + sub + '_run-01_T1map_biasCorrected_resampled_masked.nii.gz -t ' + out_dir + 'sub-' + sub + '_run-01_biasCorrected_resampled_masked_registered_to_sub-' + sub + '_run-02_biasCorrected_masked_1InverseWarp.nii.gz -t [' + out_dir + 'sub-' + sub + '_run-01_biasCorrected_resampled_masked_registered_to_sub-' + sub + '_run-02_biasCorrected_masked_0GenericAffine.mat,1]')
		map_data = os.path.join(out_dir, 'sub-' + sub + '_data_resampled_registered_to_' + sub + '_run-01_T1map_biasCorrected_resampled_masked.nii.gz')
...
if map_file_onto_surface:
	...
	print('*****************************************************')
	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled_leftHemisphere_cropped.nii.gz') and reprocess != True:
		print('File exists already. Skipping process.')
		map_data_leftHemisphere = os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled_leftHemisphere_cropped.nii.gz')
	else:
		img = nb.load(os.path.join(out_dir, 'sub-' + sub + '_merged_run-01+02_leftHemisphere_xmask-lcrgm.nii.gz'))
		tmp = img.get_fdata()
		tmp[tmp<0] = 0
		...
		nb.save(map_data_leftHemisphere, os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled_leftHemisphere_cropped.nii.gz'))
...
# ---------------------
# Here, we sample the T1 values of the T1map onto all layers. And we
# extract the layers 1 to 18, removing layers close to the cerebrospinal
...
if map_file_onto_surface:
	...
	profile = nighres.laminar.profile_sampling(
		                ...
		                overwrite=reprocess,
		                output_dir=out_dir,
		                file_name='sub-' + sub + '_data_resampled_leftHemisphere_extractedLayers')['result']
	profile = nb.load(profile)
	...
	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_data_resampled_leftHemisphere_extractedLayers-allLayers_mean.nii.gz')) and reprocess != True:
		print('File exists already. Skipping process.')
		meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_data_resampled_leftHemisphere_extractedLayers-allLayers_mean.nii.gz'))
	else:
		tmp = profile.get_fdata()[:,:,:,1:19]
		extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
		nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_data_resampled_leftHemisphere_extractedLayers-allLayers.nii.gz'))
		meanProfile = mean_img(extractedLayers)
		nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_data_resampled_leftHemisphere_extractedLayers-allLayers_mean.nii.gz'))
		del tmp
	...
	nighres.surface.surface_mesh_mapping(
		                ...
		                overwrite=reprocess,
		                output_dir=out_dir,
		                file_name='sub-' + sub + '_data_resampled_leftHemisphere_extractedLayers-allLayers.vtk')
...
if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_merged_run-01+02_T1map_rightHemisphere_cropped.nii.gz')) and reprocess != True:
	...
	if map_file_onto_surface:
		...
		nb.save(map_data_leftHemisphere, os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled_rightHemisphere_cropped.nii.gz'))
...
if map_file_onto_surface:
	...
	print('*****************************************************')
	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled_rightHemisphere_cropped.nii.gz') and reprocess != True:
		print('File exists already. Skipping process.')
		map_data_rightHemisphere = os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled_rightHemisphere_cropped.nii.gz')
	else:
		img = nb.load(os.path.join(out_dir, 'sub-' + sub + '_merged_run-01+02_rightHemisphere_xmask-rcrgm.nii.gz'))
		tmp = img.get_fdata()
		tmp[tmp<0] = 0
		...
		tmp2 = tmp1.get_fdata()
		map_data_rightHemisphere = nb.Nifti1Image(tmp2[coord], affine=tmp1.affine, header=tmp1.header)
		nb.save(map_data_rightHemisphere, os.path.join(out_dir, 'sub-' + sub + '_map_data_resampled_rightHemisphere_cropped.nii.gz'))
...
if map_file_onto_surface:
	...
	profile = nighres.laminar.profile_sampling(
		                ...
		                overwrite=reprocess,
		                output_dir=out_dir,
		                file_name='sub-' + sub + '_data_resampled_rightHemisphere_extractedLayers')['result']
	profile = nb.load(profile)
	...
	if os.path.isfile(os.path.join(out_dir, 'sub-' + sub + '_data_resampled_rightHemisphere_extractedLayers-allLayers_mean.nii.gz')) and reprocess != True:
		print('File exists already. Skipping process.')
		meanProfile = nb.load(os.path.join(out_dir, 'sub-' + sub + '_data_resampled_rightHemisphere_extractedLayers-allLayers_mean.nii.gz'))
	else:
		tmp = profile.get_fdata()[:,:,:,1:19]
		extractedLayers = nb.Nifti1Image(tmp, affine=layers.affine, header=layers.header)
		nb.save(extractedLayers, os.path.join(out_dir, 'sub-' + sub + '_data_resampled_rightHemisphere_extractedLayers-allLayers.nii.gz'))
		meanProfile = mean_img(extractedLayers)
		nb.save(meanProfile, os.path.join(out_dir, 'sub-' + sub + '_data_resampled_rightHemisphere_extractedLayers-allLayers_mean.nii.gz'))
		del tmp
	...
	nighres.surface.surface_mesh_mapping(
		                ...
		                overwrite=reprocess,
		                output_dir=out_dir,
		                file_name='sub-' + sub + '_data_resampled_rightHemisphere_extractedLayers-allLayers.vtk')
