function preproc_sensemap(data)
...
param.fwhm    = 30;        % FWHM (default: 60)
param.reg     = 0.001;     % Regularization (default: 0.001)
param.samp    = 2;         % Sampling distance (default: 3)
...
if param.mask == 1 && param.seg == 1
    ...
    HeaderInfo.fname = [path '/' name '_brainmask' ext];
    HeaderInfo.private.dat.fname = HeaderInfo.fname;

    spm_write_vol(HeaderInfo, mask);

    if param.gzip
        disp('Compressing mask and segmentation data.')
    ...
    fprintf('\n')
    fprintf('*****************************************\n')
    fprintf('* Applying brain mask to data           *\n')
    fprintf('*****************************************\n\n')
    HeaderInfo = spm_vol_nifti([path '/' name '_biasCorrected' ext]);
    img        = spm_read_vols(HeaderInfo);   

    masked_img = img .* mask;
    
    HeaderInfo.fname = [path '/' name '_biasCorrected_masked' ext];
    spm_write_vol(HeaderInfo, masked_img);

    if param.gzip
        disp('Compressing masked data.')

	gzip([path '/' name '_biasCorrected_masked' ext])
	%delete([path '/' name '_biasCorrected_masked' ext])
    end
end

if param.gzip
    disp('Compressing bias corrected data.')
    gzip([path '/' name ext])
